...
# Client variables
var server_peer: ENetConnection
...
func handle_events() -> void:
	...
	var event_type: EConnection.EventType = packet_event(0)
	
	while event_type != ENetConnection.EVENT_NONE:
		var peer: ENetPacketPeer = packet_event(1)
		
		match event_type:
			ENetConnection.EVENT.ERROR:
				push_warning("Package resulted in an unknown error")
				return
			ENetConnection.EVENT.CONNECT:
				if is_server:
					peer_connected(peer)
				else:
					connected_to_server()
			ENetConnection.EVENT.DISCONNECT:
				if is_server:
					peer_disconnected(peer)
				else:
					disconnected_from_server()
					return # Return because connection was set to null
			ENetConnection.EVENT.RECEIVE:
				if is_server:
					on_server_packet.emit(peer.get_meta("id"), peer.getpacket())
				else:
					on_client_packet.emit(peer.get_packet())
		# Call service() again to handle remaining packets to current while loop
...
func start_client(ip_address: String = "127.0.0.1", port: int = 42069) -> void:
	...
	if error:
		print("Cilent starting failed: ", error_string(error))
		connection = null
		return
...
func disconnected_to_server() -> void:
	print("Successfully disconnected to server!")
